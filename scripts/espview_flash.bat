...
set "EXTRA="
if defined NO_RESET set "EXTRA= '--extra-args=--after no-reset'"
echo [flash] running: idf.py -B build\%ESP32_PROFILE% -p %PORT% flash
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; try { & { . '%ESPIDF_PROFILE%' }; Set-Location '%ROOT%\esp32'; idf.py -B build/%ESP32_PROFILE% -p %PORT% flash%EXTRA%; if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE } } catch { Write-Host ('[flash] ERROR: ' + $_.Exception.Message); exit 6 }"
