...
//   - present(rect, pixels)：仅 Application 场景 + 启用时，经
//     shared/oled PhysicalRenderer（Agent D 交付）同步渲染进 OledDisplay
//     的共享 1KB 应用 fb（mutex 保护，绝不持有 pixels 指针）；Diagnostics
...
private:
    ...
    int srcW_ = 0;  // 源帧分辨率（init 时记录；PhysicalRenderer 的 srcW/srcH）
    int srcH_ = 0;
